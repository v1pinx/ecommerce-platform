...
export default function Navbar() {
    ...
    const categoryObj = {
        ...
        "charger": "#", // Example link for "Charger"
    };
...
const FlyoutLink = ({ children, href, FlyoutContent, category }: {
    children: any,
    href: any,
    FlyoutContent: React.FC<Category>,
    category: FlyoutContentData
}) => {
    ...
    return (
        <div
            ...
            <a className="relative text-white" href={href}>{children}
                <span
                    style={{
            ...
            <AnimatePresence>
                {showFlyout && (
                    <motion.div
                        ...
                        <FlyoutContent category={category} />
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
...
interface Category {
    [key: string]: string; // Flexible to handle multiple key-value pairs
}

const CategoryContent: React.FC<{ category: Category }> = ({ category }) => {
    // Animation Variants for the container and items
    const containerVariants = {
        hidden: { opacity: 0, y: 30 }, // Slide up slightly on entry
        visible: {
            opacity: 1,
            y: 0,
            transition: {
    ...
    const itemVariants = {
        ...
        hover: {
            scale: 1.1,
            backgroundColor: "#fff", // Gray-500 on hover
    ...
    return (
        <motion.div
            className="w-52 bg-black p-4 shadow-2xl rounded-xl"
            variants={containerVariants}
            initial="hidden"
            animate="visible"
        >
            <motion.div
                className="space-y-2 text-white flex flex-col justify-center items-center "
                variants={containerVariants}
            >
                {Object.entries(category).map(([key, url], index) => (
                    <motion.a
                        key={index}
                        href={url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block py-2 w-full text-center text-white rounded-md cursor-pointer hover:text-black"
